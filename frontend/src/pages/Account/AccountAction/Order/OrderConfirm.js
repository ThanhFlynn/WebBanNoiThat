...
class OrderConfirm extends React.Component {
    ...
    setUpdata = () => {
        fetch("/api/getConfirmOrder?fro="+this.state.start+"&to="+(this.state.start+3)+"&type=1",{
            method:'GET',
            headers:{
                        ...
                        <div className="row py-3">
                            ...
                            <div className="col-2" style={{position: "relative"}}>
                                <button style={{position: "absolute",top: "50%",left:"50%",transform: "translate(-50%,-50%)",
                                    border:"1px solid #bbb", padding: "5px 10px",backgroundColor:"red",color:"#fff",borderRadius:"5px"}}
                                    onClick={(e) => {e.preventDefault();if(window.confirm("Bạn chắc chắn muốn xóa đơn hàng này?")) this.removeOrderDetail(i[0],index);}}
                                    >
                                        Hủy
                                </button>
                            </div>
